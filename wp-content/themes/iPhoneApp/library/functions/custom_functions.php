...
$pt_metaboxes = array(
		"image" => array (
			"name"		=> "image",
			"default" 	=> "",
			"label" 	=> __("Custom Image Location"),
			"type" 		=> "text",
			"desc"      => __("Enter full URL path for image to be used by the Dynamic Image resizer. (including <code>http://</code>). Image must be uploaded to your blog or it won't resize due to copyright restrictions of TimThumb script. You also need to Chmod <code>cache</code> folder in theme files to 777 restrictions.")
		),
	);
	...
	if (!is_single()) {
		...
		if($max_page > 1 || $always_show) {
			...
			if ($paged >= ($pages_to_show-1)) {
				...
				echo '<a href="javascript:void(0);" onclick="content_loader_js(\''.get_pagenum_link().'\',\'\');">&laquo; '.get_option('ptthemes_pagination_first_name').'</a>';
				}

			}
			//previous_posts_link($prelabel);
			get_previous_posts_link_custom($prelabel);
			for($i = $paged - $half_pages_to_show; $i  <= $paged + $half_pages_to_show; $i++) {
				if ($i >= 1 && $i <= $max_page) {
					if($i == $paged) {
						...
						echo ' <a href="javascript:void(0);" onclick="content_loader_js(\''.get_pagenum_link($i).'\',\'\')">'.$i.'</a> ';
						}
					}
				}
			}
			//next_posts_link($nxtlabel, $max_page);
			get_next_posts_link_custom($nxtlabel, $max_page);
			if (($paged+$half_pages_to_show) < ($max_page)) {
				if(get_option('ptthemes_remove_pageajax')){
				echo '<a href="'.get_pagenum_link($max_page).'">'.get_option('ptthemes_pagination_last_name').' &raquo;</a>';
				}else{
				echo '<a href="javascript:void(0);"  onclick="content_loader_js(\''.get_pagenum_link($max_page).'\',\'\')">'.get_option('ptthemes_pagination_last_name').' &raquo;</a>';
				}
			}
			echo "</div> $after";
		}
